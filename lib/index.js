...
(function () {
	...
	DateSelector.prototype = {
			...
			for (var i = 0; i < Math.ceil((count + start) / 7); i++) {
				...
				for (var j = 0; j < 7; j++) {
					...
					if (nonull) {
						...
						if (this._disabledDate instanceof Function) if (this._disabledDate(new Date(datadate))) {
							cls.push('notallow');
						}
						{}
					} else {
		...
		onAddBtnClick: function onAddBtnClick(event) {
			...
			if (this._disabledDate instanceof Function) {
				if (this._disabledDate(nfrom) || this._disabledDate(nto)) {
					...
					setTimeout(function () {
						_this5.el.querySelector('.options .tip').textContent = '';
					}, 3000);
					return false;
				}
			}
		...
		onBgSCroll: function onBgSCroll() {
			self.showAt();
		},
		...
		showAt: function showAt() {
			var _self$target$getBound = self.target.getBoundingClientRect(),
			    left = _self$target$getBound.left,
			    right = _self$target$getBound.right,
			    top = _self$target$getBound.top,
			    bottom = _self$target$getBound.bottom,
			    height = _self$target$getBound.height,
			    width = _self$target$getBound.width;
		...
		show: function show() {
			...
			setTimeout(function () {
				document.body.addEventListener('click', _this6.onMarkClick);
				document.addEventListener('scroll', _this6.onBgSCroll);
			});
			this.isshow = true;
		},
		hide: function hide() {
